...
**Environment configuration** (secrets, URLs, feature flags read from `process.env`) lives in a **single typed config module** loaded at startup, never inline in services or handlers. See `rules/code-quality/env-config.md` for the full doctrine: schema validation with Zod or equivalent, fail-fast at boot, typed exports, client/server separation in frameworks that support it. Inline `process.env.X` reads in business logic are forbidden — the only allowed exceptions are `NODE_ENV`, build-time tooling, and tests with explicit cleanup.
